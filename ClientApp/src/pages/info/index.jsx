...
const InfoPage = (
  {idUltimoHorarioReal,numerosSillasReserva}
  ) => {

    const numerosSillasReserva2 = [1,11]
  ...
  const renderSeats = () => {
    ...
    return (
      <div className="seat-layout">
        <div>------------ Pantalla del cinema ------------</div>
        {seatRows.map((row) => (
          <div key={row} className="seat-row">
            {seatCols.map((col) => {
              const seatNumber = `${row}${col}`;
              const isSelected = numerosSillasReserva.includes(parseInt(seatNumber, 10)) || false; // Verificación de nulidad
