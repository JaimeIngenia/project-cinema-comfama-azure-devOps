...
const baseUrl = 'https://localhost:7240/api/pelicula/VerPelicula'
const baseUrl2 ='https://localhost:7240/api/pelicula/VerPeliculaPorId/'
...
// }
export function getCategoriaOracleBySlug(slug) {
  return fetch(`https://localhost:7240/api/pelicula/VerPeliculaPorId/${slug}`)
    .then((response) => {
      if (!response.ok) throw new Error("Network response was not ok.");
      return response.json().then((courses) => {
        if (courses.length !== 1) throw new Error("Course not found: " + slug);
